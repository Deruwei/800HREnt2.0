 ...
 */
public class SelectPlaceToJob extends BaseActivity implements
        OnClickListener {
        ...
        try {
            ...
            switch (fromTag) {// 初始化上一个页面对象
                case FROMTAG_VALUE1:// 求职意向
                    fromActivity = PostJobActivity.postJobActivity;
                    break;
                default:
                    break;
            }
                        ...
                        if (zhixiashiORhotcity.contains(dataArrayList.get(position).get("value"))) {
                            if (checkStateHashMap.containsKey(dataArrayList.get(position).get("key"))) {
                                ...
                                if (checkStateHashMap.size() == 99999) {
                                    Toast.makeText(SelectPlaceToJob.this, "数量已达最大值", Toast.LENGTH_SHORT).show();
                                    return;
                                }
                                // 设置选中状态
                            ...
                            SelectPlaceToJob placeSelectedActivity = SelectPlaceToJob.this;
                            ...
                            try {
                                if (!dataArrayList.get(position).get("value").equals("#热门城市") && !dataArrayList.get(position).get("value").equals("#按省份选择城市")) {
                                    ...
                                    adapter2 = new MyBaseAdapterFindJobPlaceSelect2(SelectPlaceToJob.this, dataArrayList2);
                    ...
                    switch (fromTag) {
                        case FROMTAG_VALUE1:// 求职意向
                            if (checkStateHashMap.containsKey(dataArrayList2.get(position).get("key"))) {
                                checkStateHashMap.remove(dataArrayList2.get(position).get("key"));
                                // 取消选中状态
                                if (dataArrayList2.get(position).get("key").endsWith("00")) {
                                    Iterator keys = checkStateHashMap.keySet().iterator();
                                    while (keys.hasNext()) {
                                ...
                                if (dataArrayList2.get(position).get("key").endsWith("00")) {
                                    ...
                                    checkStateHashMap.put(dataArrayList2.get(position).get("key"), true);
                                } else {
                                    // 如果已选title，则移除本title
                                    checkStateHashMap.remove(dataArrayList2.get(position).get("key").subSequence(0, 2) + "00");
                                    if (checkStateHashMap.size() == 99999) {
                                        Toast.makeText(SelectPlaceToJob.this, "数量已达最大值", Toast.LENGTH_SHORT).show();
                                        return;
                                    }
    ...
    private ArrayList<HashMap<String, String>> getData(String filter, String id, String value) throws Exception {
        ...
        if (id == null) { // 一级分类
            if ("place".equals(filter)) {
                ...
                if (fromTag == FROMTAG_VALUE1 && data.get(data.size() - 1).containsValue("aa00")) {// 移除aa00
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.post_placeselect_confirm:// 确定
                ...
                switch (fromTag) {
                    case FROMTAG_VALUE1:// 求职意向
                        PostJobActivity.postJobActivity.setPlaceId(checkStateHashMap);
