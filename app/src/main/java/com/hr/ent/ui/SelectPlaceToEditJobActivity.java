 ...
 */
public class SelectPlaceToEditJobActivity extends BaseActivity implements
        OnClickListener {
    ...
    private ListView add_city_listView;
    ...
    private void init() {
        try {
            ...
            mListView = (ListView) findViewById(R.id.placeselect_listview);
            add_city_listView = (ListView) findViewById(R.id.add_city);
            ...
            switch (fromTag) {// 初始化上一个页面对象
                case FROMTAG_VALUE1:// 求职意向
                    fromActivity = EditJobActivity.editJobActivity;
                    break;
                default:
                    break;
            }
                    ...
                    if (fromTag == FROMTAG_VALUE1) {// --------------------------求职意向-工作地址
                        if (zhixiashiORhotcity.contains(dataArrayList.get(position).get("value"))) {
                            if (checkStateHashMap.containsKey(dataArrayList.get(position).get("key"))) {
                                ...
                                // 检测数量
                                if (checkStateHashMap.size() == 99999) {
                                    Toast.makeText(SelectPlaceToEditJobActivity.this, "数量已达最大值", Toast.LENGTH_SHORT).show();
                                    return;
                                }
                                // 设置选中状态
                            ...
                            SelectPlaceToEditJobActivity placeSelectedActivity = SelectPlaceToEditJobActivity.this;
                            ...
                            try {
                                if (!dataArrayList.get(position).get("value").equals("#热门城市") && !dataArrayList.get(position).get("value").equals("#按省份选择城市")) {
                                    ...
                                    adapter2 = new MyBaseAdapterFindJobPlaceSelect2(SelectPlaceToEditJobActivity.this, dataArrayList2);
                        ...
                        case FROMTAG_VALUE1:// 求职意向
                            if (checkStateHashMap
                                ...
                                if (dataArrayList2.get(position)
                                    ...
                                    if (checkStateHashMap.size() == 99999) {
                                        Toast.makeText(
                                                SelectPlaceToEditJobActivity.this,
                                                "数量已达最大值",
                                                Toast.LENGTH_SHORT)
                                                .show();
                                        return;
...
//                            ResumeJobExpLVAdapter
//                                    .setPlaceId(dataArrayList2.get(
//                                            position).get("key"));
//                            ResumeJobExpLVAdapter
//                                    .setPlaceText(dataArrayList2
//                                            .get(position).get("value")
//                                            .trim());
////                            Toast.makeText(SelectPlaceToResumeActivity.this, dataArrayList2.get(position).get("value")
...
//                            ResumePersonInfoActivity.setPlaceId(dataArrayList2.get(position).get("key"));
//                            ResumePersonInfoActivity.setPlaceText(dataArrayList2.get(position).get("value").trim());
////                            Toast.makeText(SelectPlaceToResumeActivity.this, dataArrayList2.get(position).get("value")
////                                    .trim(), Toast.LENGTH_SHORT).show();
//                            break;
//                        default:
...
//            }
            ...
            if (keepState) {
                ...
                if (placeselect_showinforlayout.getVisibility() == View.VISIBLE) {
                    placeselect_showinforlayout.setVisibility(View.GONE);
                    placeselect_showinforlayout.removeAllViews();
                    placeselect_selectedinfo.setCompoundDrawablesWithIntrinsicBounds(null, null,
                            getResources().getDrawable(R.mipmap.jiantou_right), null);
                } else {
                    placeselect_showinforlayout.removeAllViews();
...
//                } else {// 使用程序内置数据
                if (isCHS) {// ------------------------------------zh
                    ...
                    JSONArray cityJSONArray = new JSONArray(
                            NetUtils.readAsString(in, Const.ENCODE));
                    data = getCityArray(cityJSONArray, id);
                } else {// ------------------------------------en
                    InputStream in = getAssets().open("city_en.json");
                    JSONArray cityJSONArray = new JSONArray(NetUtils.readAsString(in, Const.ENCODE));
                    data = getCityArray(cityJSONArray, id);
//                    }
                }
            }
            ...
            if (fromTag == FROMTAG_VALUE1) {// 求职意向-工作地点
                ...
                if (zhixiashiORhotcity.contains(dataArrayList.get(position)
                    ...
                    if (checkStateHashMap.containsKey(dataArrayList.get(
                         ...
                         * 防止重用
                         */
                        String cityName = dataArrayList.get(position).get("value").trim();
                        if (cityName.equals("北京") || cityName.equals("上海") || cityName.equals("天津") || cityName.equals("重庆")) {
                            holder.name.setCompoundDrawablesWithIntrinsicBounds(null, null, getResources().getDrawable(R.mipmap.duihao), null);
                        }
                    } else {// 未勾选状态
                        holder.name.setCompoundDrawablesWithIntrinsicBounds(null, null, null, null);
                    }
                ...
                if (viewType == 0) {// 定位
                    if (convertView == null) {
                        ...
                        convertView = inflater.inflate(R.layout.placeselect_listview_item, null);
                        topViewHolder.title = (TextView) convertView.findViewById(R.id.placeselect_listview_item_title);
                        topViewHolder.title.setVisibility(View.VISIBLE);
                        topViewHolder.name = (TextView) convertView.findViewById(R.id.placeselect_listview_item_text);
                        topViewHolder.name.setCompoundDrawablesWithIntrinsicBounds(null, null, null, null);
...
//                        }
//                    } else {
//                        topViewHolder.name.setText(isCHS == true ? "定位失败" : "Position failed");
                    topViewHolder.name.setText("北京");
//                    }
//                    topViewHolder.title.setText(isCHS == true ? "定位城市" : "Position city");
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.post_placeselect_confirm:// 确定
                ...
                switch (fromTag) {
                    case FROMTAG_VALUE1:// 求职意向
                        EditJobActivity.editJobActivity.setPlaceId(checkStateHashMap);
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (fromTag == FROMTAG_VALUE1) {// 求职意向-工作地点
                if (convertView == null) {
                    convertView = inflater.inflate(R.layout.placeselect_listview_item, null);
                    holder = new ViewHolder();
                    holder.title = (TextView) convertView.findViewById(R.id.placeselect_listview_item_title);
                    holder.name = (TextView) convertView.findViewById(R.id.placeselect_listview_item_text);
                    convertView.setTag(holder);
                } else {
                    holder = (ViewHolder) convertView.getTag();
                    holder.name.setVisibility(View.VISIBLE);
                ...
                holder.title.setVisibility(View.GONE);
                if (checkStateHashMap.containsKey(dataArrayList2.get(position).get("key"))) {//
