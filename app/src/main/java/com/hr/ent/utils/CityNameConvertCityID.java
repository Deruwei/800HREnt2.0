...
//                Log.i(TAG, "======= 下面的都没运行 ");
                ...
                cityJSONArray = new JSONArray(NetUtils.readAsString(in, Const.ENCODE));
    ...
    private static ArrayList<HashMap<String, String>> getArrayList(JSONArray jsonArray)
...
//        in = context.getAssets().open("city_zh.json");
//        cityJSONArray = new JSONArray(NetUtils.readAsString(in, Const.ENCODE));
////        Log.i(TAG, "======= in " + cityJSONArray.toString());
//        return getArrayList(cityJSONArray);
//    }

//    public static String getCityName(String cityId, Context context) throws Exception {
//        InputStream in = null;
//        JSONArray cityJSONArray = null;
//        in = context.getAssets().open("city_zh.json");
//        cityJSONArray = new JSONArray(NetUtils.readAsString(in, Const.ENCODE));
//        ArrayList<HashMap<String, String>> data = getCityArray(cityJSONArray, "0");
//        for (HashMap<String, String> map : data) {
//            if (!map.get(cityId).equals("") || map.get(cityId) != null) {
...
//    }
    ...
    public static String convertCityName(Context context, String cityId) {
//        Toast.makeText(context, cityName.toString(), Toast.LENGTH_SHORT).show();
...
//                Log.i(TAG, "======= in " + in.toString());
                ...
                ArrayList<HashMap<String, String>> data = getCityArray(cityJSONArray, "0");
                for (HashMap<String, String> map : data) {
                    if ((map.get("key").trim()).equals(cityId.trim())) {
                        String cityName = map.get("value");
                        return cityName;
                    }
                }
            } catch (JSONException e) {
