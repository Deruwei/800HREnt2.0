...
public class Log_Location {
	 ...
	 *             抛出读写异常
	 */
	public static void logToSDcard(String name, long sendtime, Object location,
			Context context) {
		try {
	...
	private static void writeLog(String path, long sendtime, Object location)
		...
		String SDPath = Environment.getExternalStorageDirectory()
				+ Const.LOGPATH
				+ DateUtils.transforMillToDate(System.currentTimeMillis());// flyox/log/当天日期/
		String ROOTPath = Environment.getExternalStorageDirectory()
				+ Const.LOGPATH;// 日志保存的根目录
	 ...
	 *             抛出的读写异常
	 */
	private static void saveLoaction(String path, long sendtime, Object location)
			throws IOException {
	...
	private static String getLocationmsg(long sendtime, Object location) {
		...
		if (location instanceof Location) {
			...
			content = "'"
					+ DateUtils
							.transforMillToDateInfo(location2.getTime() / 1000)
					+ "\t" + locmsg;
		} else if (location instanceof String) {
			content = "'"
					+ DateUtils.transforMillToDateInfo(System
