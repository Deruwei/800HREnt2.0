...
public class MyCustomDatePicker {
    ...
    private int startYear, startMonth, startDay,endYear, endMonth=12, endDay;
    private int  selectYear,selectMonth,selectDay;
    private int  spanYear, spanMon, spanDay;//最后日期
    private int startMonth1=1, startDay1=1;//开始日期
    ...
    public MyCustomDatePicker(Context context, ResultHandler resultHandler) {
            ...
            SimpleDateFormat formatter = new SimpleDateFormat("yyyy年MM月dd");//格式为 2013年9月3日 14:44
            Date curDate = new Date(System.currentTimeMillis());//获取当前时间
            String  currentDate = formatter.format(curDate);
            startYear=Integer.parseInt(currentDate.substring(0,currentDate.indexOf("年")));
            startMonth=Integer.parseInt(currentDate.substring(currentDate.indexOf("年")+1,currentDate.indexOf("月")));
            startDay=Integer.parseInt(currentDate.substring(currentDate.indexOf("月")+1));
            initDialog();
            initView();
    }
            ...
            public void onClick(View view) {
                if("".equals(textValue)||textValue==null){
                    if(typeNum==3) {
                        ...
                        String m=f2.format(selectMonth);
                        String d=f2.format(selectDay);
                        handler.handle(selectYear + "-" + m+"-"+d);
                    }else{
    ...
    private void initTimer(int type) {
        ...
        for (int i = startMonth1; i <= endMonth; i++) {
            month.add(String.valueOf(i));
        }
        if(selectYear==spanYear&&selectMonth==spanMon) {
            if (spanDay == startYear && spanMon == startMonth) {
                startDay1=startDay;
                endDay=spanDay;
            } else {
                startDay1 = 1;
        ...
        }else if(selectYear==startYear&&selectMonth==startMonth){
            if (spanDay == startYear && spanMon == startMonth) {
                startDay1=startDay;
                endDay=spanDay;
            } else {
        ...
        for (int i = startDay1; i <= endDay; i++) {
    ...
    private void dayChange() {
        ...
        //Log.i("天",day.toString());
        //Log.i("天当前选择的",startDay1+"---"+endDay+"---");
        day_pv.setData(day);
        if(selectDay>endDay) {
            day_pv.setSelected(0);
            //Log.i("天当前选择的",startDay1+"---"+endDay+"---1");
            ...
            //Log.i("天当前选择的",startDay1+"---"+endDay+"---2");
        }
    ...
    public void show(String time,String endtime,int type) {
        if (canAccess) {
            typeNum=type;
            if (isValidDate(time, "yyyy-MM-dd")) {
                ...
                String  currentDate = endtime;
                endYear=Integer.valueOf(currentDate.substring(0,currentDate.indexOf("-")));
                endMonth=Integer.valueOf(currentDate.substring(currentDate.indexOf("-")+1,currentDate.lastIndexOf("-")));
                endDay=Integer.valueOf(currentDate.substring(currentDate.lastIndexOf("-")+1));
                spanDay=endDay;
                spanYear=endYear;
                spanMon=endMonth;
               // Log.i("当前选择的",selectDay+"---"+endYear+"---"+endMonth+"---"+endDay);
                canAccess = true;
                initTimer(type);
                addListener();
